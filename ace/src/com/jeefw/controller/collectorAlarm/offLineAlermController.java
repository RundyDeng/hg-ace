...
@Controller
@RequestMapping("/collectorAlarm/offLineAlerm")
public class offLineAlermController extends IbaseController {
	@Resource
	...
	public void getWarningSearch(HttpServletRequest request, HttpServletResponse response) throws IOException{
		...
		setFiltersStr(getFilters().replace("\"field\":\"AREAGUID\"", "\"field\":\"AREAGUID\"")
								//  .replace("\"field\":\"ISYES\"", "\"field\":\"AREAGUID\"")
								  .replace("\"field\":\"ISYES\"", "\"field\":\"ISYES\"")
								);
				
		if(!getFilters().contains("\"field\":\"AREAGUID\",\"op\":\"eq\",\"data\":\"\"")){
			sql = "select * from VOFFLINE "+getSqlWhere() ;
			System.out.println("第一"+sql);
		}else if(!getFilters().contains("\"field\":\"ISYES\",\"op\":\"eq\",\"data\":\"\"")){
			sql = "select * from VOFFLINE "+getSqlWhere();
			System.out.println("第二"+sql);
		}else{
			sql = "select * from VOFFLINE ";
			System.out.println("第三"+sql);
		}*/
	...
	@RequestMapping("/getOffLinestatus")
	public void getWarningStatus(HttpServletRequest request,HttpServletResponse response) throws IOException{
	//	List<Tfailurecode> list = ((Service<Tfailurecode>) todayDataService).queryByProerties("areaguid", getParm("areaguid"));
		
		String sql = "select n.areaname ,t.GPRSID,t.ISYES,t.zxdate from TGPRS t left join tarea n on t.areaguid=n.areaguid ";
	...
	public void updateBaseInfoSearch(HttpServletRequest request , HttpServletResponse response){
		...
		if("excel".equals(oper)){
			...
			//String areaname = todayDataService.getAreanameById(getParm("areaguid"))+"离线告警表";
		//	 String areaname = "在离线报警表"+getParm("date");
			//areaname = areaname.replaceAll(" ", "");
		//	String excSql  = "select areaname,DADDRESS,GPRSID,ISYES,zxdate from VOFFLINE where areaguid="+getParm("areaguid");//+" order by areaname" 
