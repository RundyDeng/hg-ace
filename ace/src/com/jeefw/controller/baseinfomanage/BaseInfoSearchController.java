...
public class BaseInfoSearchController extends IbaseController {
	...
	public void updateBaseInfoSearch(HttpServletRequest request , HttpServletResponse response){
		...
		if("excel".equals(oper)){
			...
			String excSql = "select WYID,AREAGUID,AREANAME,BUILDCODE,BUILDNAME,UNITNO,FLOORNO,DOORNAME,METERID,CLIENTNO"//METERID,CLIENTNO
					+ " ,METERSEQ,GPRSID,COMMTYPE,REMARK,DEVICETYPECHILDNO,NO1 "//METERSEQ,POOLADDR,CHANNELNO,
            ...
            HSSFCell cell;
            if(list.size()>0){
            	String strTitles = "唯一ID，小区号，小区名称，楼宇号，楼宇名称，单元号，楼层号，门牌，热表号，用户编号" //热表号，用户编号
            			+ "，表序，交换机地址，通讯类型，设备备注，表类型，故障参数";// 表序，集中器地址，通道号，
