	...
	$('#exportExcel').click(function(){
		...
		if($.inArray("999", arr)>-1){ //选择了全部小区
			if($('#selType').val()=="0"){ //选择导出所有 getAllAreaAllData //&&date1="+date1+"&date2="+date2"
				var form = "<form name='csvexportform' action='${contextPath}/dataAnalysis/EnergyExpExcel/getAllAreaAllData?type=0' method='post'>";
			 	   form = form + "<input type='hidden' name='csvBuffer' value='" + encodeURIComponent(1) + "'>";
			 	   form = form + "</form><script>document.csvexportform.submit();</sc" + "ript>";
			 	  
			}else if($('#selType').val()=="1"){ //选择导出故障数据
				var form = "<form name='csvexportform' action='${contextPath}/dataAnalysis/EnergyExpExcel/getAllAreaAllData?type=1' method='post'>";
			 	   form = form + "<input type='hidden' name='csvBuffer' value='" + encodeURIComponent(1) + "'>";
			 	   form = form + "</form><script>document.csvexportform.submit();</sc" + "ript>";  
			}
