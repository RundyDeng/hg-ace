...
<br/>
		<div class="row" style="height: 30px;">
			...
			<div class="col-md-11" >
				...
				<div class="input-group" >
					<span class="input-group-addon" id="basic-addon3" >楼栋1</span> 
					<select id="build" onchange="uniting(this)" class="form-control" style="width: 70px;">
					</select>
				</div>
				<div class="input-group" >
					<span class="input-group-addon" id="basic-addon3" >单元1</span> 
					<select	id="unit" class="form-control" style="width: 70px;">
					</select>
				</div>
				
				<div class="input-group">
					  <span class="input-group-addon"  id="basic-addon3">房间位置</span>
					   <select name="position" id="position" class="form-control"  style="width: 70px;">
					   		 <option>请选择</option>
...
%>
				...
				<div class="input-group">
					<span class="input-group-addon" id="basic-addon3">楼栋2</span> <select
						id="build2" onchange="uniting2(this)" class="form-control"  style="width: 70px;"></select>
				</div>
				<div class="input-group">
					<span class="input-group-addon" id="basic-addon3">单元2</span> <select
						id="unit2" class="form-control"  style="width: 70px;"></select>
				</div>
				
				<div class="input-group">
					  <span class="input-group-addon"  id="basic-addon3">房间位置</span>
					   <select name="position2" id="position2" class="form-control" style="width: 70px;">
					   		 <option>请选择</option>
					   		 <option value=1>顶层</option>
					   		 <option value=2>顶边层</option>
					   		<option value=3>边层</option>
...
var _areaguid,_buildno,_unitno;
var _areaguid2,_buildno2,_unitno2;
...
$('#build_data_search').click(function(){
    ...
    var unitno = $("#unit").val();
    var unitno2 = $("#unit2").val();
    var position = $('#position').val();
    var position2 = $('#position2').val();
	 if(position=="请选择" || position2=="请选择"){
		 position='1';
		 position2='1';
	 }
    if(unitno=="请选择" || position2=="请选择"){
    	unitno="1"; unitno2="1";
    }
	if(areaguid==null){
		areaguid='2032';
    ...
    $.get('${contextPath}/dataanalysis/buildpositioncontr/getbuildposition?areaguid='+areaguid+'&startdate='+startDate+'&enddate='+endDate
    		+'&buildno='+buildno+'&buildno2='+buildno2+'&areaguid2='+areaguid2+'&unitno='+unitno+'&unitno2='+unitno2+'&position='+position+'&position2='+position2, function(response){
    ...
    var showTable = function(objs){
      ...
      //   var td4 = '<td>单位系数</td>';
        ...
        $(objs).each(function(index,obj){
            array_build_name.push(obj.UNITNO);
            th += '<th>'+obj.UNITNO+'单元'+'</th>';
            td1 += '<td>'+roundFix(obj.BUILD_ENERGY)+'</td>';
            td2 += '<td>'+obj.CLIENT_TOTLE+'</td>';
            
       //     td4 += '<td>'+obj.METERXISHID+'</td>';
