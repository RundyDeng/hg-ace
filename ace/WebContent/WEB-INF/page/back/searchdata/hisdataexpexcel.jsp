...
if(attrDate!=""&&attrDate!=null){
	today = (String)request.getParameter("edate");
 	weekAgo = (String)request.getParameter("sdate");
}else{
...
<div class="row">
 <div class="col-xs-9">
  <table style="width: 100%;" class="table table" cellpadding="0" cellspacing="0">
   ...
   <td>
   <label for="lblMessage1">已下载</label>
   </td>
   </tr>
   <tr>
   <td>
   <select size="4" name="selArea" onClick=""
      id="selArea" style="height: 200px; width: 180px;" multiple="multiple"></select>
   </td>
   <td>
   ...
   <select size="4" name="uploadedArea" onClick=""
      id="uploadedArea" style="height: 200px; width: 180px;"></select>
   </td>
   </tr>
...
$('.page-content-area').ace_ajax('loadScripts',scripts,function() {
	...
	$('#exportExcel').click(function(){
		...
		if($.inArray("999", arr)>-1){ //选择了全部小区
			if($('#selType').val()=="0"){ //选择导出所有 getAllAreaAllData //&&date1="+date1+"&date2="+date2"
				var form = "<form name='csvexportform' action='${contextPath}/searchData/HisdataExpExcel/getAllAreaAllData?type=0&&date1="+date1+"&date2="+date2+" ' method='post'>";
			 	   form = form + "<input type='hidden' name='csvBuffer' value='" + encodeURIComponent(1) + "'>";
			 	   form = form + "</form><script>document.csvexportform.submit();</sc" + "ript>";
			 	  
		 	  
			}else if($('#selType').val()=="1"){ //选择导出故障数据
				var form = "<form name='csvexportform' action='${contextPath}/searchData/HisdataExpExcel/getAllAreaAllData?type=1&&date1="+date1+"&date2="+date2+" ' method='post'>";
			 	   form = form + "<input type='hidden' name='csvBuffer' value='" + encodeURIComponent(1) + "'>";
			 	   form = form + "</form><script>document.csvexportform.submit();</sc" + "ript>";  
			}
