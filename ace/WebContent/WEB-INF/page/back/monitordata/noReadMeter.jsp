        ...
        $('.page-content-area').ace_ajax('loadScripts', scripts, function() {
        	jQuery(function($) {
        		...
        		jQuery(grid_selector).jqGrid({
        			subGrid : false,
        			url : "${contextPath}/monitorData/noReadMeterContr/getNoReadMeter",
        			datatype : "json",
        			height : 450,
        			rownumbers:true,
        			rownumWidth:40,
        			colNames : ['小区编号','楼宇','单元','楼层','表类型','故障类型',     
        			            '用户编码','小区名称', '地址', '表编号','集中器地址','通道号','日期','抄表批次'],
        			colModel : [{
	        			            hidden:true,
	       			            	label:'小区名',
	       			            	name:'AREAGUID',
       			             	...
       			             	},          
       			            	{
        	        				...
        	        				formatter : function(cellvalue, options, rowObject){
        	        					return rowObject.ADDRESS.replace('号','');//.replace('栋','')
        	        				},
        	        				search : true,
        	        				stype:'select',
        	        				searchoptions:{
        		...
        		function submitExcel(){
      			   ...
      			   $.get('${contextPath}/monitorData/noReadMeterContr/updateNoReadMeter?oper=excel&flag=1&date='+date+'&areaguid='+areaguid,function(data){
      				   if(data==1){
      					   alert('没有数据！');
      				   }else{
      					   var form = "<form name='csvexportform' action='${contextPath}/monitorData/noReadMeterContr/updateNoReadMeter?oper=excel&date="+date+"&areaguid="+areaguid+"' method='post'>";
