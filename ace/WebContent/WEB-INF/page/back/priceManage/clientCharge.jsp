        	...
        	jQuery(function($) {
        		...
        		var template1 = { 
        				...
        				"rules": [ 
							...
							//		 { "field": "CLIENTNAME", "op": "eq", "data": ""},
							//		 { "field": "SFTR", "op": "eq", "data": ""}, 
									 { "field": "JCJG", "op": "eq", "data": "5.7"},
									 { "field": "JLJG", "op": "eq", "data": "0.142"},
									 { "field": "MJJG", "op": "eq", "data": "19"},
        		...
        		jQuery(grid_selector).jqGrid({
        			subGrid : false,
        			url : "${contextPath}/priceMange/clientChargeContr/getClientCharge",
        			mtype:'POST',
        			datatype : "json",
        			postData: urlParm,
        			height : 370,
        			rownumbers:true,
        			rownumWidth:40,
        			colNames : ['小区编号','楼宇','单元','楼层', '小区名称','用户编码', '门牌号','住户姓名', '用热状态','供热面积(㎡)'
        			            ,'起始日期','起始热量(kwh)','终止日期','终止热量(kwh)'
        			            ,'用热量(kwh)','基础价格(元)	','基础热费(元)','计量价格(元/kwh)','计量热费(元) ','热费合计(元)','面积单价(元/㎡)','面积价格(元)','日期','退费额(元)'],
        			colModel : [ 
  {
    hidden:true,
   	label:'小区名',
        		...
        		if(true/* <shiro:hasPermission name="${ROLE_KEY}:todaydata:export">true</shiro:hasPermission><shiro:lacksPermission name="${ROLE_KEY}:todaydata:export">false</shiro:lacksPermission> */){
     			...
     			function submitExcel(){
     			  // var currentDate = $("#input_sel_date").val();
     			   ...
     			   if(areaguid=='')
     				   return;
     			   $.get('${contextPath}/priceMange/clientChargeContr/updateClientCharge?oper=excel&flag=1&date1='+date1+"&date2="+date2+"&areaguid="  + areaguid,function(data){
     				   if(data==1){
     					   alert('没有当日数据！');
     				   }else{
     					  var form = "<form name='csvexportform' action='${contextPath}/priceMange/clientChargeContr/updateClientCharge?oper=excel&&date1="+date1+"&date2="+date2+"&areaguid=" + areaguid+"' method='post'>";
