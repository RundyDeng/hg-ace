			...
			<div class="modal-content">
				...
				<div class="modal-footer no-margin-top">
					<div class="text-center">
						<button id="submitButton" type="button" class="btn btn-app btn-success btn-xs" disabled="true">
...
<script type="text/javascript">
		function checkPo2(){
			var treeLength = $('#authorityTree div[class="tree-loader"]').length;
			if(treeLength > 0) setTimeout("checkPo2();",200);
			else{
				window.frames.length != parent.frames.length ? $(window.parent.document).find("#zq-preloaderbar").removeClass("zq-ajax") : $("#zq-preloaderbar").removeClass("zq-ajax");
				$('#submitButton').attr('disabled',false);
			}
		}
		function checkPo(){
			$('#submitButton').attr('disabled',true);
			$('#authorityTree div[class="tree-loader"]').length == 0 ? setTimeout("checkPo();",200) : checkPo2();
		};
        ...
        $('.page-content-area').ace_ajax('loadScripts', scripts, function() {
        	...
        	jQuery(function($) {
        		...
        		function authorityFormatter(cellvalue, options, cell) {																			//ROLE_ADMIN
        			var template = "<button data-toggle='modal' onclick='javascript:$(\"#modal-table\").modal(\"toggle\");$(\"#roleKeyId\").val(\"" + cell.roleKey 
        					+"\");window.frames.length != parent.frames.length ? $(window.parent.document).find(\"#zq-preloaderbar\").addClass(\"zq-ajax\") : $(\"#zq-preloaderbar\").addClass(\"zq-ajax\");checkPo();' class='btn btn-xs btn-warning'><i class='ace-icon fa fa-flag bigger-120'></i></button>";
        			return template;
        		}
        		...
        		$('#submitButton').on('click', function() {
        			if($('#authorityTree div[class="tree-loader"]').length > 0){
        				alert("请等待权限树加载完毕！")
        				return ;
        			}
					var output = '';
        		...
        		var template1 = { 
        				"groupOp": "AND", 
        				"rules": [ 
									 { "field": "roleKey", "op": "eq", "data": "" } , 
									 { "field": "roleValue", "op": "cn", "data": "" } ,
        		...
        		jQuery(grid_selector).jqGrid('navGrid', pager_selector, { // navbar options
        			...
        			view : <shiro:hasPermission name="${ROLE_KEY}:role:view">false</shiro:hasPermission><shiro:lacksPermission name="${ROLE_KEY}:role:view">false</shiro:lacksPermission>,
