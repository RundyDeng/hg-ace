...
(function(factory) {
	...
	Tree.prototype = {
				...
				$.each(items.data, function(index, value) {
					...
					if (value.type === 'folder') {
						...
						if ('children' in value){
							if(value['children'] === true) header.find('i').hide();
						}
						...
						$($entity[0]).find('span i').replaceWith('<input type="radio" class="icon-item" value="'+value.id+'" name="xxii"/>');
					}
		...
		selectItem : function(el) {
			...
			$(el).find('input[name="xxii"]').get(0).checked= true;
