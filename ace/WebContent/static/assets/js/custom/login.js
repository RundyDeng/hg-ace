...
jQuery(function($) {
	...
	$('#validationRetrieveForm').validate({
		...
		submitHandler : function(form) {
			$.ajax({
				dataType : "json",
				url : getContextPath() + "/sys/sysuser/retrievePassword", // 发送邮件找回密码
				type : "post",
				data : {
	...
	$('#validationLoginForm').validate({
		...
		submitHandler : function(form) {
			$.ajax({
				...
				type : "post",
				data : {
					userName : $('#loginUserName').val(),  //email loginEmail 9/18
				...
				complete : function(xmlRequest) {
					var returninfo = eval("(" + xmlRequest.responseText + ")");
					if (returninfo.result == 1) {
						//document.location.href = getContextPath() + "/sys/sysuser/home?ub="+userBrowser();
