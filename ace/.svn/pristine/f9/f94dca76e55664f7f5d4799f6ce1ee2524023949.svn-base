...
(function($ , undefined) {
	...
	function AceAjax(contentArea, options) {
		...
		this.loadUrl = function(hash) {
			var url = false;
			hash = hash.replace(/^(\#\!)?\#/, '');
			if(typeof content_url === 'function') url = content_url(hash);  //  /ace/sys/sysuser/homepage
...
///////////////////			
					...
					if(link_element.length > 0) {
						...
						if(nav.length > 0) {
							...
							nav.closest('.sidebar[data-sidebar-scroll=true]').each(function() {
								...
								if(manual_trigger) $this.ace_sidebar_scroll('scroll_to_active');
							})
						}
					}
				}
		...
		this.startLoading = function() {
			$('#zq-preloaderbar').addClass('zq-ajax');
		...
		this.stopLoading = function(stopNow) {
			if(stopNow === true) {
				...
				$('#zq-preloaderbar').removeClass('zq-ajax');
			}
			else {
		...
		.on('hashchange.ace_ajax', function(e, manual_trigger) {  
			...
			if(curUrl.indexOf("sys/sysuser/home")>0 && !(window.location.hash)){   
				self.loadUrl(default_url); return;
			}else{  //为iframe
				if(!window.frameElement) return;;
				if(curUrl.indexOf("sys/sysuser/baseframe")>0){
					self.getUrl(curUrl.replace(/baseframe$/,window.frameElement.id),'',false);
					return;
				}
				self.getUrl(document.location,'',false);
			}
	...
	$.fn.aceAjax = $.fn.ace_ajax = function (option, value, value2) {
		...
		if(!window.frameElement)  {
			...
			var initDiv = '<div class="main-container" id="main-container"> \
			...
			if($(".page-content-area").length == 0 ) {
				//$(window.document.body).addClass("zq-skin").append(initDiv);
			}
			
			$set = $(".page-content-area").each(function () {
				var $this = $(this);
				var data = $this.data('ace_ajax');
				var options = typeof option === 'object' && option;
				if (!data) $this.data('ace_ajax', (data = new AceAjax(this, options)));
