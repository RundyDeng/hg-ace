...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		beginReq = function() {
			...
			if(!$('#zq-preloaderbar').length) $(window.parent.document).find('#zq-preloaderbar').addClass('zq-ajax');
		},
		endReq = function() {
			ts.grid.hDiv.loading = false;
			$(ts).jqGrid("progressBar", {method:"hide", loadtype : ts.p.loadui });
			$('#zq-preloaderbar').removeClass('zq-ajax');
			if(!$('#zq-preloaderbar').length) $(window.parent.document).find('#zq-preloaderbar').removeClass('zq-ajax');
		},
...
$.jgrid.extend({
	...
	getRowData : function( rowid ) {
		...
		this.each(function(){
			var $t = this,nm,ind;//nm--->XXX---> colModel {name :  XXX}
			if(rowid === undefined) {
				getall = true;
				resall = [];
				len = $t.rows.length;
						...
						if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
							if($t.p.treeGrid===true && nm === $t.p.ExpandColumn) {
								...
								try {
                                   ...
                                   /* console.log('j:' + j +'   rowId:'+ind.id+'   colModel:'+$t.p.colModel[i] + '   i:'+i)
                                    console.log(nm +':'+res[nm])*/
								} catch (e){
