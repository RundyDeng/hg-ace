...
(function($ , undefined) {
	...
	var handle_on_change = function() {
		...
		var fileName = file.name;// = this.element.value
		var fileSize = file.size;
		var fileType = file.type;
		var fileLastModifiedDate = file.lastModifiedDate;
		fileName = fileName.split('.')[0];
		$('#form-field-recipient-zq') && $('#form-field-recipient-zq').val(fileName);
		$('#form-field-subject-zq') && $('#form-field-subject-zq').val(fileType);
