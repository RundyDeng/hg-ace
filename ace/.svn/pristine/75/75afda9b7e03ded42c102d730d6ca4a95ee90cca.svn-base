...
<div class="row">
	<div class="col-xs-12">
		<div id="downlistBar">
			<table style="width: 100%" class="table">
				<tbody>
						...
						<td>
							搂栋低区：<input id="Radio1" type="radio" name="banlance" value="4" />							
						</td>
						<td >
							单元分析：
					...
					<tr>
						<td colspan="7" style="border-bottom: 1px solid #ddd;">
							<button type="button" value="ZL" class="btn btn-info">总量平衡分析</button>
						  	<button type="button" value="excel" class="btn btn-info">导出EXCEL</button>
						 	<button type="button" value="home" class="btn btn-info">返回首页</button>						 	
						</td>
					</tr>
				</tbody>
			</table>
		...
		<div id="each_charts">
				...
				<img src="../../../../ace/static/image/zl.png">
	</div>
</div>
<%-- <script
        		...
        		$.get(load_url, function (csv) {
 				   ...
 				   $('#each_charts').highcharts({
 					  ...
 					  yAxis: [{ // left y axis
 			           ...
 			           plotOptions: { area: { fillColor: { linearGradient: { x1: 0, y1: 0, x2: 0, y2: 1}, stops: [ [0, Highcharts.getOptions().colors[0]], [1, Highcharts.Color(Highcharts.getOptions().colors[0]).setOpacity(0).get('rgba')] ] }, lineWidth: 1, marker: { enabled: false }, shadow: false, states: { hover: { lineWidth: 1 } }, threshold: null } }, 
 			            //数据点配置
 			           /*  plotOptions: {
 			                series: {
 			                    cursor: 'pointer',
